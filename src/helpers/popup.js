...
export function initPickerPopup(parent, options, data, Popup) {
    ...
    mixins: [
      {
        created() {
          ...
          data.forEach(({ propName, parentName, watch }) => {
            if (watch) {
              this.$watch(`$parent.${parentName}`, val => {
                this[propName] = val
              })
            }
          })
  ...
  data.forEach(({ propName, parentName }) => {
    propsData[propName] = parent[parentName]
  })
