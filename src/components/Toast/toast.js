...
function render(options) {
  const html = [
    `<div class="toast ` + (options.image || (options.icon && options.icon !== 'none') ? 'has-icon' : '') + `">`,
    (options.mask ? `<div class="toast-mask"></div>` : ''),
    `<div class="toast-box">`,
    (options.image ? `<img class="toast-icon" src="${options.image}" />` : (options.icon === 'success' ? `<i class="toast-icon">
