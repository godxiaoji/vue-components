...
function render(options) {
  const html = [
    ...
    `<button style="display: ` + (options.showCancel ? 'block' : 'none') + `;" class="modal-button modal-cancel-button">${options.cancelText}</button>`,
...
export function show(object) {
  ...
  try {
    ...
    options.maskClosable = getParamValue(object, 'maskClosable', 'boolean', false)
...
export function hide() {
  if ($el) {
    // 添加事件
    $el.querySelector('.modal-cancel-button').removeEventListener('click', Events.onCancelClick, false)
    $el.querySelector('.modal-confirm-button').removeEventListener('click', Events.onConfirmClick, false)
